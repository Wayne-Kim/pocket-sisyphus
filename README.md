...
<!-- 이 목록의 SSOT 는 에이전트 픽커다: iOS `ios/PocketSisyphus/Models/AgentKind.swift` +
     ...
     이 4개 표면의 정합은 `scripts/agent-surfaces-lint.sh` 가 픽커 SSOT 와 대조해 점검한다. -->
...
> 대상이 아니다. 커뮤니티는 별도로 만들지 않고 (경쟁 제품들 관행대로) **Discord** 로 운영한다.
> 적용 범위는 아래 [프로젝트 경계](#프로젝트-경계) 참고.
...
커뮤니티 기능은 **만들지 않기로 했다** — 경쟁 제품들 관행대로 **Discord** 로 대신한다 (저장소 밖 외부 플랫폼이라 «인프라 0» 과 무관).
...
- [위협 모델](docs/THREAT_MODEL.md) — 자산·신뢰 경계·완화·수용된 잔여 위험
- [능력 캡 가드레일](docs/CAPABILITY_CAPS.md) — 개인-데이터 경로의 lethal trifecta 차단 명세
- [보안 정책 · 취약점 신고](docs/SECURITY.md) — 지원 버전·신고 절차·응답 SLA (`/.well-known/security.txt`)
...
> 운영 문서는 비공개 `docs/ops/` 에 보관한다. 일반 사용자 설치는 README 위쪽 「설치」 섹션 참고.
...
> 메인테이너 «인프라» 비용은 두 앱 기준 $0/yr (→ [핵심 원칙](#핵심-원칙)). 웹(소개 사이트)은 외부 호스팅을 쓰고 커뮤니티는 Discord 로 운영하므로 별개 — [프로젝트 경계](#프로젝트-경계) 참고.

## 라이선스 · 기여

**비공개(private) 저장소이며 독점 라이선스다 — 오픈소스가 아니다.** 전문은 [`LICENSE.md`](LICENSE.md).

- ✅ (접근 권한자) 소스 열람·클론, 본인 PC에서 직접 빌드해 **개인적·비상업적** 사용, 본인용 수정, 기여(PR) 목적 수정.
