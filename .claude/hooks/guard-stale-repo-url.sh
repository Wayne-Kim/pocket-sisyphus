...
#   github.com/Wayne-Kim/pocket-claude 를 가리켜 404 가 났다. 이제 소스·배포·가이드가
#   모두 공개 저장소 github.com/Wayne-Kim/pocket-sisyphus 한 곳에 있다(install.sh·DMG
#   릴리스·web 전부 여기). pocket-sisyphus-mac 은 전환기 Sparkle 미러일 뿐 — 새 사용자
#   링크에는 pocket-sisyphus 를 쓴다.
#
...
if grep -nE 'github\.com/Wayne-Kim/pocket-claude' "$file_path" >/dev/null 2>&1; then
  hits="$(grep -nE 'github\.com/Wayne-Kim/pocket-claude' "$file_path")"
  {
    ...
    echo "   공개 저장소(소스·배포·가이드)는 github.com/Wayne-Kim/pocket-sisyphus 입니다."
    echo "   파일: $file_path"
    echo "$hits" | sed 's/^/     /'
    echo "   → pocket-claude 를 pocket-sisyphus 로 고치세요."
  } >&2
  exit 2
fi
