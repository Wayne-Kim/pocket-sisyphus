...
import { readConfig, writeConfig, CONFIG_DIR, type DaemonConfig } from "./config.js";
import { generateToken, hashToken } from "./auth.js";
import {
  startTor,
  getActiveTorProcess,
  kickTorReconnect,
  getStoredOnionAddress,
  getTorBootstrapPercent,
  getLastTorReconnectAt,
  type TorHandle,
} from "./tor/sidecar.js";
...
import { startSsh, SSH_PORT, isSshProcessAlive, type SshHandle } from "./ssh/server.js";
import {
  ensureHostKey,
...
import {
  ...
  getLastWanIpChangeAt,
} from "./nat/external-ip.js";
import { getLanHosts } from "./nat/lan-addr.js";
import { reclaimStaleDaemon } from "./reclaim.js";
import { findAvailablePort } from "./ports.js";
import { makeLogger, UNIFIED_LOG_FILE } from "./logging/log.js";
import { connectionDiagnosticsRoute } from "./routes/connection-diagnostics.js";
...
export async function start(opts: StartOptions = {}): Promise<void> {
  ...
  // daemon 종료 시 정리할 watcher/스케줄러 stop 콜백 등록부. 각 부팅 단계 헬퍼가 채운다.
  const watcherStops: Array<() => void> = [];

  // === 부팅 단계 — 실행 순서대로 (각 단계 정의는 아래 헬퍼) ===
  const app = buildHttpApp();
  startBackgroundWatchers();
  const { port, ENDPOINT_PORT } = await resolveListenPorts();
  const server = serve(
  ...
  // ---- 부팅 단계 헬퍼 (위 호출 순서가 곧 실행 순서) ----

  function loadOrInitConfig(): DaemonConfig {
  ...
  function buildHttpApp(): Hono {
    ...
    // 진단 번들 — iOS 「문제 신고/진단」 화면이 묶어 공유할 로컬 진단(서브시스템 스냅샷 + 최근
    // crash 마커 + 마스킹된 unified.log tail). 읽기 전용·자동 전송 없음(LAN 전용·무텔레메트리).
    app.route("/api/diagnostics", diagnostics);

    // 연결 진단 — 서브시스템 «읽기 전용» 스냅샷 (connection_diagnostics_v1). Tor/sshd/외부 연결성/
    // 에이전트 CLI/디스크/로그/네트워크 상태를 안정적 코드(connection-diagnostics/codes.ts)로 내보내,
    // iOS 「연결 진단」 화면이 원인·권장 조치를 안내한다. (별개의 /api/diagnostics 는 «문제 신고/진단
    // 번들».) 핸들/모듈 상태를 «요청 시점» 에 읽는 getter 로 주입 — endpoint 라우트와 같은 패턴
    // (핸들은 serve 콜백에서 늦게 채워지지만 closure 가 요청 때 읽는다). torHandle/sshHandle 선언
    // «뒤» 에 mount 해 TDZ 를 피한다.
    app.route(
      "/api/connection-diagnostics",
      connectionDiagnosticsRoute({
        getTorProcessAlive: () => {
          const p = getActiveTorProcess();
          return p !== null && !p.killed;
        },
        getTorBootstrapPercent,
        // onion 게시 신호 — 활성 핸들의 주소(부팅 후) 또는 저장된 hostname 파일. 둘 다 없으면
        // 미게시(콜드부트에서 onion 미준비) → tor_descriptor_missing.
        getOnionAddress: () => torHandle?.onionAddress ?? getStoredOnionAddress(),
        getSshListening: () => sshHandle !== null && isSshProcessAlive(),
        getSshPort: () => sshHandle?.port ?? sshPortPreferred,
        getDiskFree: () => {
          try {
            const s = fs.statfsSync(CONFIG_DIR);
            return {
        ...
        // 등록된 각 에이전트 어댑터의 CLI 가 시스템에 있는지 — resolveBinary throw = 미설치.
        listAgentCli: () =>
          listAgents().map((a) => {
            let detected = true;
            try {
        ...
        getLanCandidateCount: () => {
          try {
            return getLanHosts().length;
          } catch {
            return 0;
          }
        },
        getUnifiedLogBytes: () => {
          try {
            return fs.statSync(UNIFIED_LOG_FILE).size;
          } catch {
            return null;
          }
        },
        getPtyChunkCount: () => {
          try {
            const row = db()
              .prepare("SELECT COUNT(*) AS n FROM messages WHERE type = 'pty_chunk'")
        ...
        // 외부 IPv4 는 «존재 여부» 만 — 값 자체는 진단에 불필요하고 로그 redact 정책과 일관.
        getExternalIPv4: () => getCachedExternalIPv4()?.ipv4 ?? null,
        getIpFetchedAt: () => getCachedExternalIPv4()?.fetchedAt ?? null,
        getLastIpChangeAt: () => getLastWanIpChangeAt(),
        getLastReconnectAt: () => getLastTorReconnectAt(),
        now: () => Date.now(),
      }),
    );
  ...
  function startBackgroundWatchers(): void {
  ...
  async function resolveListenPorts(): Promise<{ port: number; ENDPOINT_PORT: number }> {
    ...
    return { port, ENDPOINT_PORT };
  }

  async function onDaemonListening(info: AddressInfo): Promise<void> {
    ...
    runAgentCliSelfCheck();
    startLocalSchedulers();
    await startNetworkServices(info);
  }

  function recordRuntimeInfo(info: AddressInfo): void {
  ...
  function runAgentCliSelfCheck(): void {
  ...
  function startLocalSchedulers(): void {
  ...
  async function startNetworkServices(info: AddressInfo): Promise<void> {
    // === SSH 서버 + Tor 듀얼 채널 부팅 ===
    //
  ...
  function setupWebSocketServer(server: ReturnType<typeof serve>): void {
