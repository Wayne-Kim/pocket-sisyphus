...
`,
  ...
  // ── 기본(default/bug) 본문 ───────────────────────────────────────────────────
  "collect.defaultBody": {
    ar: `أنت وكيل مالك المنتج (PO) لهذا المستودع. مهمتك أن تجد «العمل الجدير بالبناء التالي» في هذا المستودع وتنظّمه في بريفات فرص. لا تعدّل الكود — اقرأ/ابحث فقط.
...
إن لم يوجد فعلاً ما تقترحه فاكتب مصفوفة فارغة []. بعد كتابة الملف، أنهِ بسطر واحد «اكتمل كتابة N بريف».{{outputDirective}}`,
    en: `You are this repository's Product Owner (PO) agent. Your mission is to find the «next valuable thing to build» in this repo and organize it into opportunity briefs. Do not modify code — only read/investigate.
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
...
If there is truly nothing to propose, write an empty array []. After writing the file, end with one line: «N briefs written».{{outputDirective}}`,
    es: `Eres el agente Product Owner (PO) de este repositorio. Tu misión es encontrar «lo siguiente valioso por construir» en este repo y organizarlo en briefs de oportunidad. No modifiques código — solo lee/investiga.
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
...
Si de verdad no hay nada que proponer, escribe un array vacío []. Tras escribir el archivo, termina con una línea: «N briefs escritos».{{outputDirective}}`,
    fr: `Tu es l'agent Product Owner (PO) de ce dépôt. Ta mission est de trouver «la prochaine chose utile à construire» dans ce dépôt et de l'organiser en briefs d'opportunité. Ne modifie pas le code — lis/investigue seulement.
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
...
S'il n'y a vraiment rien à proposer, écris un tableau vide []. Après avoir écrit le fichier, termine par une ligne: «N briefs écrits».{{outputDirective}}`,
    hi: `आप इस रिपॉज़िटरी के Product Owner (PO) एजेंट हैं। आपका कार्य इस रेपो में «अगली बनाने योग्य मूल्यवान चीज़» खोजकर उसे अवसर-ब्रीफ़ में संगठित करना है। कोड न बदलें — केवल पढ़ें/जाँचें।
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
...
यदि वास्तव में प्रस्तावित करने को कुछ न हो तो खाली array [] लिखें। फ़ाइल लिखने के बाद एक पंक्ति «N ब्रीफ़ लिखे गए» से समाप्त करें।{{outputDirective}}`,
    ja: `あなたはこのリポジトリのプロダクトオーナー(PO)エージェントだ。このリポジトリの「次に作る価値ある仕事」を見つけ、機会ブリーフに整理するのが任務だ。コードを修正するな — 読む/調べるだけ。
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
...
提案すべきものが本当になければ空配列 [] を書け。ファイルを書いたら「N件のブリーフ作成完了」の一行で終えよ。{{outputDirective}}`,
    ko: `너는 이 저장소의 프로덕트 오너(PO) 에이전트다. 이 레포의 «다음에 만들 가치 있는 일» 을 찾아 기회 브리프로 정리하는 것이 임무다. 코드를 수정하지 마라 — 읽기/조사만 한다.
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
...
제안할 것이 정말 없으면 빈 배열 [] 을 써라. 파일을 쓴 뒤 «브리프 N건 작성 완료» 한 줄로 끝내라.{{outputDirective}}`,
    "pt-BR": `Você é o agente Product Owner (PO) deste repositório. Sua missão é encontrar «a próxima coisa valiosa a construir» neste repo e organizá-la em briefs de oportunidade. Não modifique código — apenas leia/investigue.
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
...
Se realmente não houver nada a propor, escreva um array vazio []. Após escrever o arquivo, termine com uma linha: «N briefs escritos».{{outputDirective}}`,
    ru: `Ты — агент Product Owner (PO) этого репозитория. Твоя задача — найти «следующую ценную вещь для создания» в этом репозитории и оформить её в брифы возможностей. Не изменяй код — только читай/исследуй.
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
...
Если действительно нечего предложить, запиши пустой массив []. После записи файла закончи одной строкой: «Написано N брифов».{{outputDirective}}`,
    "zh-Hans": `你是本仓库的产品负责人(PO)智能体。你的任务是在本仓库中找到「下一个值得构建的有价值之事」,并整理为机会简报。不要修改代码——只读取/调研。
{{profile}}{{directive}}{{history}}{{verification}}{{storeReviews}}{{crashSignals}}{{designContext}}{{lensBlock}}
