...
describe("requireLocalAdmin 미들웨어 — ASC .p8 키 등 로컬 운영자 전용 자산 게이트", () => {
  // §5.10(A5): ASC .p8 키는 A3 페어링(폰)과 «분리된» 로컬 운영자 전용 자산이어야 한다. 폰은
  // Bearer·attest 를 쥐어도 localAdminSecret(QR/페어링 번들 미포함)이 없어 X-PS-Local 을 못
  // 만든다 → 거부. requireAttestation 의 X-PS-Local 이 «면제 우회»(폰도 안 막음)였던 것과 정반대로,
  // 이 게이트는 그 비밀이 없는 모든 호출자를 막는다. /asc-key(GET/PUT/DELETE) + /asc-key/verify 가 대상.
  const LOCAL = "local-admin-secret-fixed-for-test";
  let pub: string;
  let app: Hono;

  beforeEach(() => {
    pub = rawP256PubKey(makeKeyPair().publicKey);
    app = new Hono();
    // 실제 라우터(routes/po.ts)의 등록과 동형 — 정확한 /asc-key 와 하위 /asc-key/*.
    app.use("/asc-key", requireLocalAdmin);
    app.use("/asc-key/*", requireLocalAdmin);
    app.get("/asc-key", (c) => c.json({ ok: true }));
    app.put("/asc-key", (c) => c.json({ ok: true }));
    app.delete("/asc-key", (c) => c.json({ ok: true }));
    app.post("/asc-key/verify", (c) => c.json({ ok: true }));
    // 게이트 밖 라우트 — 같은 라우터의 다른 경로엔 절대 번지지 않아야 한다(회귀 방지).
    app.get("/briefs", (c) => c.json({ ok: true }));
  });
  afterEach(() => setConfig({}));

  it("X-PS-Local 이 localAdminSecret 과 일치하면 통과 (같은 머신의 Mac 앱 운영자)", async () => {
    setConfig({ localAdminSecret: LOCAL });
    const res = await app.request("/asc-key", { headers: { "x-ps-local": LOCAL } });
    expect(res.status).toBe(200);
  });

  it("X-PS-Local 헤더가 없으면 403 local_admin_required — 폰 Bearer 보유자 거부", async () => {
    setConfig({ localAdminSecret: LOCAL });
    const res = await app.request("/asc-key");
    expect(res.status).toBe(403);
    expect(((await res.json()) as { error: string }).error).toBe("local_admin_required");
  });

  it("X-PS-Local 불일치는 403", async () => {
    setConfig({ localAdminSecret: LOCAL });
    const res = await app.request("/asc-key", { headers: { "x-ps-local": "wrong-secret" } });
    expect(res.status).toBe(403);
  });

  it("유효 attest 토큰만 있고 X-PS-Local 이 없으면 거부 — 폰 정상 경로도 키 라우트는 못 만진다", async () => {
    // 폰은 페어링(enrolled) 후 X-PS-Attest 로 일반 /api/* 는 통과하지만, 키 라우트는 이 게이트가 따로 막는다.
    setConfig({ attestPublicKey: pub, localAdminSecret: LOCAL });
    const { token } = issueAttestToken(pub);
    const res = await app.request("/asc-key", { headers: { "x-ps-attest": token } });
    expect(res.status).toBe(403);
  });

  it("GET/PUT/DELETE/verify 모두 같은 게이트 — 헤더 없으면 전부 403, 일치하면 전부 통과", async () => {
    setConfig({ localAdminSecret: LOCAL });
    const ok = { headers: { "x-ps-local": LOCAL } };
    expect((await app.request("/asc-key")).status).toBe(403);
    expect((await app.request("/asc-key", { method: "PUT", ...ok })).status).toBe(200);
    expect((await app.request("/asc-key", { method: "DELETE" })).status).toBe(403);
    expect((await app.request("/asc-key", { method: "DELETE", ...ok })).status).toBe(200);
    expect((await app.request("/asc-key/verify", { method: "POST" })).status).toBe(403);
    expect((await app.request("/asc-key/verify", { method: "POST", ...ok })).status).toBe(200);
  });

  it("게이트 밖 라우트(/briefs)엔 번지지 않는다 — 헤더 없이도 통과 (폰 회귀 0)", async () => {
    setConfig({ localAdminSecret: LOCAL });
    expect((await app.request("/briefs")).status).toBe(200);
  });

  it("localAdminSecret 미설정이면 fail-closed — 일치시킬 값이 없어 어떤 헤더도 거부", async () => {
    setConfig({}); // localAdminSecret 없음
    expect((await app.request("/asc-key", { headers: { "x-ps-local": LOCAL } })).status).toBe(403);
  });
});
