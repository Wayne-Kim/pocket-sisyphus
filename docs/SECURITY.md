...
전체 위협 모델·신뢰 경계·수용된 잔여 위험은 [`docs/THREAT_MODEL.md`](docs/THREAT_MODEL.md),
방어 구현은 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) (§4 보안 모델 · §8 알려진 리스크), 개인-데이터
경로의 능력 캡(lethal trifecta 차단)은 [`docs/CAPABILITY_CAPS.md`](docs/CAPABILITY_CAPS.md) 참고.
...
유지한다([README — Versioning policy](README.md#versioning-policy)). 보안 수정은 **최신 릴리즈로 전진
...
- 재현 절차 / PoC, 영향(어떤 자산 — [THREAT_MODEL §2](docs/THREAT_MODEL.md) 참고)과 전제 조건.
- 가능하면 제안하는 완화.
...
- **iOS 앱** (`ios/`) 과 **Mac 앱 + daemon** (`mac/`): 전송 평면(직접 SSH / Tor onion / bridge),
  ...
  비밀 보관(Keychain) 등 — [THREAT_MODEL](docs/THREAT_MODEL.md) 의 자산·경계 전반.

**Out-of-scope** — 보고 대상 아님:
- **수용된 잔여 위험** — 사용자 자신의 에이전트 실행 파괴성, 터미널 ANSI 렌더 표면, single-hop onion 의
  서버 익명성 포기 등. 의도된 설계이며 [THREAT_MODEL §6](docs/THREAT_MODEL.md) 에 명시돼 있습니다.
