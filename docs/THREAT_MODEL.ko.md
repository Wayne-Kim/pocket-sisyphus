[English](THREAT_MODEL.md) · **한국어**

# 위협 모델 — Pocket Sisyphus

> 이 문서는 «무엇을 지키는가 / 무엇을 신뢰하는가 / 무엇을 막고 무엇은 못 막는가» 를 명시한다.
> 구현 세부는 [ARCHITECTURE.ko.md](ARCHITECTURE.ko.md) (특히 §4 보안 모델 · §8 알려진 리스크) 가 SSOT 이고,
> 취약점 신고 절차는 [SECURITY.ko.md](SECURITY.ko.md) 다. 이 문서는 그 둘을 «위협 모델»
> 관점으로 묶는다.
>
> 개인-데이터 경로(향후 메일·캘린더 등)에 적용할 **능력 캡 가드레일** 명세는
> [CAPABILITY_CAPS.ko.md](CAPABILITY_CAPS.ko.md) 가 정본이다 — 본 문서 §5.8(공격면)·§6(잔여 위험)과 1:1 로 묶인다.
...
[README 의 프로젝트 경계](../README.ko.md#프로젝트-경계) 와 동일하게, 이 저장소엔 세 프로젝트가 있고 위협 모델의
무게중심이 다르다.
...
인젝션 → lethal trifecta** 공격면을 연다(§5.8). 그 경로에 강제할 능력 캡은 [CAPABILITY_CAPS.ko.md](CAPABILITY_CAPS.ko.md)
...
[ARCHITECTURE.ko.md §4](ARCHITECTURE.ko.md#4-보안-모델) 표를 정본으로 한다.
...
- **완화: 세 다리 중 ③(외부 통신)을 캡으로 끊는다** ([CAPABILITY_CAPS.ko.md](CAPABILITY_CAPS.ko.md) 정본):
...
7. **간접 프롬프트 인젝션의 «읽힘» 은 못 막는다.** 개인-데이터 경로(메일·캘린더)가 공격자 통제 콘텐츠를
   ...
   **능력 캡으로 끊어**([CAPABILITY_CAPS.ko.md](CAPABILITY_CAPS.ko.md)) «유출 완성» 을 막는 데까지다 — 오염 세션
...
책임 있는 신고(coordinated disclosure) 경로는 [SECURITY.ko.md](SECURITY.ko.md) 를 따른다 — GitHub
